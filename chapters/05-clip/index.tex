\section{CLIP}
\textbf{CLIP} (Contrastive Language-Image Pretraining) is a SOTA \textbf{Vision-Language-Model (VLM)}, designed to learn a joint embedding space for images and text, 
...
\begin{equation}
    ...
    \underbrace{\log \frac{\exp(\simop(v_i, t_i) / \tau)}{\sum_{j=1}^{N} \exp(\simop(v_i, t_j) / \tau)}}_{\textit{Image} \to \textit{Text}}
    +
    \underbrace{\log \frac{\exp(\simop(t_i, v_i) / \tau)}{\sum_{j=1}^{N} \exp(\simop(t_i, v_j) / \tau)}}_{\textit{Text} \to \textit{Image}}\right),
\end{equation}

where $v_i$ and $t_i$ are the embeddings of the $i$-th image and text pair, $\simop$ is the cosine similarity function, and $\tau$ is a \textbf{temperature} hyperparameter that controls the sharpness of the distribution.
...
\termdef{Temperature}{The temperature $\tau$ is a hyperparameter that controls the sharpness of the distribution in the contrastive loss.
...
\termdef{Training Phase}{Since the WebImageText dataset is very large, both the image and text encoders are trained from scratch, without using any pretraining on other datasets.
