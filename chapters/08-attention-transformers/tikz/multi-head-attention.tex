\begin{tikzpicture}[
    ...
    steplbl/.style={font=\footnotesize, text=dlgray!80!black, align=left, anchor=north west},
    ...
    % ===== Step captions =====
    \node[steplbl] at (-0.2,2.0)  {1) Input\\$\;\;\;$sentence $X$};
    \node[steplbl] at (2.9,2.0)   {2) Weight matrices\\$\;\;\;$per head};
    \node[steplbl] at (5.7,2.0)   {3) Attention\\$\;\;\;$output $Z_i$};
    ...
    % ===== Step 4 caption =====
    \node[steplbl] at (-0.2,-6.5)
        {4) Concatenate all $Z_i$ and project with $W^{O}$\\$\;\;\;$to obtain the final output $Z$};
