...
The learned mapping $f(\boldsymbol{x})$ is defined as a composition of several functions, each representing a layer of the network.
\begin{figure}[H]
    \centering
    \scalebox{0.8}{
        \input{chapters/02-ffnn/tikz/3-layer-FFNN.tex}
    }
...
$\boldsymbol{x} \rightarrow \phi(\boldsymbol{x})$, where $\phi$ is a non-linear function.
We can then apply the same linear model as before to the transformed data $\phi(\boldsymbol{x})$. 
