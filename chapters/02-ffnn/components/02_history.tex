...
\begin{figure}[H]
    \centering
    \scalebox{0.8}{
        \input{chapters/02-ffnn/tikz/electronic_brain.tex}
    }
...
\begin{figure}[H]
    \centering
    \scalebox{0.8}{
        \input{chapters/02-ffnn/tikz/perceptron.tex}
    }
...
\begin{figure}[H]
    \centering
    \scalebox{0.8}{
        \input{chapters/02-ffnn/tikz/mlp.tex}
    }
...
\begin{enumerate}
    ...
    \item \textbf{Loss computation}: Compute the loss $L(y, f(\vct{x}, \vct{\theta}))$, where $y$ is the true label and $f(\vct{x}, \vct{\theta})$ is the output of the network with parameters $\vct{\theta}$;
