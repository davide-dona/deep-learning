...
The usual approach to image captioning involves using a CNN to extract features from the image,
and then using an RNN to generate the caption based on those features.
...
\begin{figure}[H]
    \centering
    \resizebox{\textwidth}{!}{\input{chapters/10-sequence-models/tikz/show-and-tell.tex}}
...
\begin{figure}[H]
    \centering
    \input{chapters/10-sequence-models/tikz/show-attend-tell.tex}
    \caption{Show, Attend and Tell: at each step the decoder produces an attention map $\alpha_t$ over the CNN feature map, forming a context vector $\vct{z}_t$ that, together with the previous word, drives the prediction $p(y_t)$.}
\end{figure}

The architecture of \textit{Show, Attend and Tell} consists of three main components.

\paragraph{Encoder}
Similar to \textit{Show and Tell}, a CNN is used for feature extraction.
...
Instead, features are extracted directly from the final convolutional layer, producing a set of spatial feature vectors $\{\vct{a}_1, \vct{a}_2, \dots, \vct{a}_L\}$.
Here, $L$ represents the number of regions in the image (e.g., $14 \times 14 = 196$), and each $\vct{a}_i$ corresponds to the features of a specific region.

\paragraph{Attention Mechanism}
At each time step $t$, an attention network $f_{att}$ determines where the model should focus.
To do this, it first computes a raw relevance score for every region $i$, conditioned on the decoder's previous hidden state $\vct{h}_{t-1}$ and the corresponding feature vector $\vct{a}_i$:
\[
e_{t,i} = f_{att}(\vct{a}_i, \vct{h}_{t-1})
\]
where $f_{att}$ is typically a small MLP. These scores are then normalized across all $L$ regions with a softmax, yielding the \textbf{attention weights}:
\[
\alpha_{t,i} = \frac{\exp(e_{t,i})}{\sum_{k=1}^{L} \exp(e_{t,k})}
\]

A \textbf{context vector} $\vct{z}_t$ is then computed as a function of the attention weights $\alpha_{t,i}$ and the corresponding feature vectors $\vct{a}_i$.
In the deterministic (soft attention) case, this function is simply a weighted sum:
\[
\vct{z}_t = \sum_{i=1}^{L} \alpha_{t,i} \, \vct{a}_i
\]
though it could also be a more complex non-linear transformation.
This context vector dynamically captures the specific visual information relevant for generating the current word.

\paragraph{Decoder}
An LSTM generates the caption sequentially.
At time step $t$, the decoder receives the dynamic context vector $\vct{z}_t$, the previous hidden state $\vct{h}_{t-1}$, and the previously generated word $y_{t-1}$.
...
By conditioning the generation on $\vct{z}_t$, the decoder ensures that each word is grounded in the most relevant visual features of the image.
Since $\vct{h}_t$ is in turn used to compute the attention scores at the next time step, the region of the image the model attends to naturally shifts as the caption is generated.
...
The forward and backward hidden states are concatenated to form a single, context-rich representation for each time step of the input sequence: $\vct{h}_i = [\overrightarrow{\vct{h}}_i; \overleftarrow{\vct{h}}_i]$.

During the decoding step $t$, the model calculates an attention weight $\alpha_{t,i}$ for each encoder hidden state $\vct{h}_i$.
These weights are derived by scoring the alignment between the decoder's previous hidden state $\vct{s}_{t-1}$ and each encoder state $\vct{h}_i$, through an attention network $f_{att}$, the \textbf{alignment model}, which is trained jointly with the rest of the architecture:
\[
e_{t,i} = f_{att}(\vct{s}_{t-1}, \vct{h}_i)
\]
Note that $\vct{s}_{t-1}$ is used here out of necessity rather than choice: in this architecture the context vector $\vct{c}_t$ is itself fed into the recurrence that produces $\vct{s}_t$, so at the point where the attention scores are computed, $\vct{s}_t$ does not exist yet, only $\vct{s}_{t-1}$ is available. This is in contrast with the local attention architecture presented later, where the decoder state is computed \textit{before} attention and is therefore free to use $\vct{s}_t$ directly.

These scores are then normalized across all $T_x$ encoder states with a softmax, yielding the attention weights:
\[
\alpha_{t,i} = \softmax(e_{t,i}) = \frac{\exp(e_{t,i})}{\sum_{k=1}^{T_x} \exp(e_{t,k})}
\]

The dynamic context vector $\vct{c}_t$ is then constructed as the weighted sum of all encoder hidden states, scaled by their respective attention weights:
\[
\vct{c}_t = \sum_{i=1}^{T_x} \alpha_{t,i} \vct{h}_i
\]

\begin{figure}[H]
    \centering
    \input{chapters/10-sequence-models/tikz/rnn-attention.tex}
    \caption{RNN with attention: a bidirectional encoder produces hidden states $\vct{h}_j$; alignment weights $\alpha_{t,j}$ combine them into the context vector $\vct{c}_t$ that feeds the decoder state $\vct{s}_t$ generating $y_t$.}
\end{figure}
...
\paragraph{A two-stage hidden state}
Unlike the global attention architecture above, here the context vector is \textit{not} an input to the decoder recurrence. Instead, the decoder first computes a preliminary hidden state $\vct{s}_t$ with a standard RNN step, using only $\vct{s}_{t-1}$ and the previous output $y_{t-1}$. Since this $\vct{s}_t$ is already available, it can be used directly as the query for the rest of the mechanism: first to predict the aligned position $p_t$, and then to score the encoder states inside the resulting window, as described next. Only once the context vector has been formed from this window is $\vct{s}_t$ corrected into its final form.

\begin{figure}[H]
    \centering
    \input{chapters/10-sequence-models/tikz/local-attention.tex}
    \caption{Local attention with predictive alignment. The decoder state $\vct{s}_t$ predicts the aligned position $p_t$, which centers a fixed window over the encoder states $\vct{h}_i$. The context vector $\vct{c}_t$ is formed from the window only and combined with $\vct{s}_t$ into the attentional hidden state $\tilde{\vct{s}}_t$ used to emit $y_t$.}
\end{figure}

In the \textbf{predictive alignment} variant, $p_t$ is computed from the decoder hidden state $\vct{s}_t$ through a small learned network:
\[
p_t = T_x \cdot \sigma\!\left(\vct{v}_p^\top \tanh(\mat{W}_p \vct{s}_t)\right)
\]
where $\mat{W}_p$ and $\vct{v}_p$ are learnable parameters.
Since $\sigma(\cdot) \in (0, 1)$, multiplying by the source length $T_x$ rescales the output into a valid position within the input sequence.

The window alignment is enforced by weighting the standard attention scores with a \textbf{Gaussian} centered on $p_t$:
\[
\alpha_{t,i} = \softmax(\vct{h}_i^\top \mat{W}_a \vct{s}_t) \cdot \exp\!\left(-\frac{(i - p_t)^2}{2\sigma^2}\right)
\]
where $\mat{W}_a$ is a learnable scoring matrix and the Gaussian standard deviation is typically set to $\sigma = D/2$.
...
The Gaussian weighting is what resolves this difficulty: because $p_t$ is a real-valued quantity that appears \textit{inside} the smooth exponential term, the alignment remains differentiable with respect to $p_t$, and therefore with respect to $\mat{W}_p$ and $\vct{v}_p$.
...
\paragraph{Correcting the proposal}
Once the context vector $\vct{c}_t$ has been formed, it is used to \textit{refine} the preliminary state $\vct{s}_t$: the two are concatenated and passed through a $\tanh$ layer to produce the \textbf{attentional hidden state}
\[
\tilde{\vct{s}}_t = \tanh\!\left(\mat{W}_c [\vct{c}_t; \vct{s}_t]\right)
\]
which is what is actually used to emit $y_t$. In other words, $\vct{s}_t$ is a first proposal for the decoder state, computed with no knowledge of the source alignment, and $\tilde{\vct{s}}_t$ is the corrected version that incorporates it.

\begin{figure}[H]
    \centering
    \input{chapters/10-sequence-models/tikz/local-vs-global.tex}
    \caption{Local vs global attention. Global attention scores all encoder states $\vct{h}_i$ at every step, whereas local attention restricts the context to a window centered on the predicted position $p_t$.}
\end{figure}
