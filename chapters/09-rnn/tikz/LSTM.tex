...
\node[draw, circle, s=1cm, label={[label distance=1mm]above:$W_g$}, fill=gatecandidate] (c1) {};
\node[draw, circle, s=.5cm, right=1.5cm of c1] (p1) {\huge.};
\node[draw, circle, s=1cm, label={[label distance=1mm]right:$W_i$}, above=1.5cm of p1, fill=gateinput] (c2) {$i_t$};
\node[draw, circle, s=1cm, label={[label distance=1mm]above:Cell}, right=1.5cm of p1, fill=gatecell] (c3) {$c_t$};
\node[draw, circle, s=.5cm, below=.5cm of c3] (p2) {$\boldsymbol{+}$};
\node[draw, circle, s=1cm, below=.5cm of p2, label={[label distance=1mm]left:$W_f$}, fill=gateinput] (c4) {$f_t$};
\node[draw, circle, s=1cm, label={[label distance=1mm]above:}, right=1.5cm of c3] (c5) {};
\node[draw, circle, s=.5cm, right=1.5cm of c5] (p3) {\huge.};
\node[draw, circle, s=1cm, label={[label distance=1mm]right:$W_o$}, above=1.5cm of p3, fill=gateoutput] (c6) {$o_t$};



% Arrows
\draw[->] (c1) -- (p1) node[midway, below=.5cm, font=\Large] {$\tilde{C}_t$};
\draw[->] (c2) -- (p1);
\draw[->] (p1)--(c3);
\draw[->] (c3)--(c5);
\draw[->] (c4)--(p2);
\draw[->] (c5)--(p3);
\draw[->] (c6)--(p3);
\draw[->] (p3)--node[pos=1,right] {$h_t$} node[below=2mm] {} node[pos=.55,above=7mm,dlgreen] {} +(1.8,0);
% \draw[->] (p3)--node[pos=1,right] {$h_t$} node[below=2mm] {$h_t=\textcolor{dlgreen}{o_t} \odot \tanh c_t$} node[pos=.55,above=7mm,dlgreen] {\footnotesize$o_t=\sigma\left(W_o\binom{x_t}{h_{t-1}}+b_o\right)$} +(3,0);
