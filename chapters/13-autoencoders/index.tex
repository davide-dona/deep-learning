...
\subsubsection{Decomposing the Log-Likelihood}
...
\begin{itemize}
    ...
    \item The third term is the Kullback--Leibler divergence between the approximate posterior $q_\phi(z \mid x)$ and the true posterior $p_\theta(z \mid x)$. This term is itself \textbf{uncomputable}: evaluating it requires knowing the true posterior $p_\theta(z \mid x)$, which, as noted above, depends on the intractable marginal likelihood $p_\theta(x)$, the very quantity we were trying to avoid computing in the first place.
\end{itemize}

\subsubsection{The Evidence Lower Bound (ELBO)}

Although the third term cannot be evaluated directly, the Kullback--Leibler divergence is by definition always non-negative. It follows that the sum of the first two terms is a lower bound on the log-likelihood, known as the \textbf{Evidence Lower Bound} (ELBO):
...
The encoder and decoder are trained jointly by maximizing the ELBO (Equation~\ref{eq:ELBO}) using gradient-based optimization, balancing the reconstruction and regularization terms described above.
